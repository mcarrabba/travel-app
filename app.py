...
def check_buffer_overlap(new_date, new_buf_start, new_buf_end):
  conn = sqlite3.connect(DB_NAME)
  cursor = conn.cursor()
  cursor.execute(
      "SELECT title, buffer_start, buffer_end FROM appointments WHERE date = ?",
      (new_date,),
  )
...
with st.sidebar:
  ...
  conn = sqlite3.connect(DB_NAME)
  sidebar_df = pd.read_sql_query("SELECT price, expenses FROM appointments", conn)
  conn.close()
...
st.write(
    "Streamline your business travel with automated **30-minute buffer"
    " blackouts**, profit tracking, interactive maps, weather forecasts, and"
    " calendar exports."
)
      ...
      if submitted:
        ...
        if start_dt >= end_dt:
          ...
          has_overlap, conflicting_title = check_buffer_overlap(
              date_str, buf_start_str, buf_end_str
          )
          ...
          if has_overlap:
            ...
            cursor.execute(
                """
                            INSERT INTO appointments (title, city, phone_number, price, expenses, date, start_time, end_time, buffer_start, buffer_end)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """,
                (
                    title,
...
with tab2:
  ...
  budget_df = pd.read_sql_query(
      "SELECT city, price, expenses FROM appointments", conn
  )
  conn.close()

  if budget_df.empty:
    st.info(
        "No appointments found. Add revenue and expense data to view financial"
        " breakdowns!"
    )
  else:
...
with tab3:
  ...
  map_df = pd.read_sql_query(
      "SELECT title, city, date, start_time FROM appointments", conn
  )
  conn.close()

  if map_df.empty:
    st.info("No appointments scheduled yet. Add a destination city to view it on the map!")
  else:
    ...
    map_df["latitude"] = map_df["city"].map(lambda x: city_coords.get(x, {}).get("lat"))
    map_df["longitude"] = map_df["city"].map(lambda x: city_coords.get(x, {}).get("lon"))
    ...
    if valid_map_df.empty:
      ...
      st.map(valid_map_df, latitude="latitude", longitude="longitude", size=30, zoom=4)
      st.caption("Pins represent your appointment cities extracted automatically from your schedule.")
      ...
      if weather_data:
        ...
        with w_col1:
          st.metric(
              ...
              value=f"{weather_data['high_f']}°F ({weather_data['high_c']}°C)",
          )
        with w_col2:
          st.metric(
